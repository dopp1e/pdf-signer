...
By default the application says that the key must have a name and recommends setting a password for the encryption of the private key.
An empty string is _technically_ a valid password, but it is rather unsafe, hence the recommendation.
